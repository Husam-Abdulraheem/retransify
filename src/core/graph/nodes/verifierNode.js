...
export async function verifierNode(state) {
  ...
  const isRecheck = (state.installAttempts || 0) > 0;
  printSubStep(
    isRecheck
      ? 'Verifying AST constraints (Re-check)...'
  ...
  const isNativeWind =
    state.facts?.tech?.styling === 'NativeWind' ||
    state.facts?.tech?.styling === 'Tailwind' ||
    (state.installedPackages || []).includes('nativewind') ||
    /className\s*=/.test(generatedCode) ||
    /from ['"]nativewind['"]/.test(generatedCode);
  ...
  if (isNativeWind) {
    project.createSourceFile(
      ...
      // 🧠 Smart Global Augmentation:
      // Tells TypeScript to accept 'className', 'tw', and gracefully handle
      // excess properties on ALL JSX elements (React Native, Expo, 3rd party).
      // This eliminates TS2322 excess property errors for ALL dynamic styling props 
      // without needing hardcoded component lists, while keeping strict type-checks for known props.
  ...
  try {
    ...
    printWarning(`ts-morph crashed: ${error.message}`);
    errors.push(`Critical Syntax Error: Could not parse file.`);
  }

  if (errors.length === 0 && missingPkgs.length === 0) {
    printSubStep('Verified ✔');
  } else if (missingPkgs.length > 0) {
    printSubStep(`Missing module detected: ${missingPkgs.join(', ')}`);
  } else {
    printSubStep(
      `Verification failed: ${errors.length} constraint error(s) detected`
    );
  }
...
function analyzeASTForErrors(sourceFile, filePath, pathMap = {}) {
  ...
  if (isComponent) {
    ...
    // 3. 🚨 Dead Links Validator (Dynamic Aware)
        ...
        if (hrefAttr && hrefAttr.getKind() === SyntaxKind.JsxAttribute) {
          ...
          if (
            ...
            if (!isMatch) {
              ...
              errors.push(
                `Line ${line}: CRITICAL ROUTING ERROR. Dead link detected. The route "${val}" DOES NOT EXIST in Expo paths. Valid raw routes are: [${available}]. You MUST update the 'href' to a valid route.`
              );
            }
          }
        }
