...
export class Planner {
  ...
  async plan(context, files) {
    ...
    if (files) {
      ...
      console.log(
        `📍 Generated Smart Paths for ${Object.keys(pathMap).length} files.`
      );
    }
    ...
    if (
      context.decisions.mutationRequests &&
      context.decisions.mutationRequests.length > 0
    ) {
      const lastRequest =
        context.decisions.mutationRequests[
          context.decisions.mutationRequests.length - 1
        ];
      console.log(
        `🔄 Planner received mutation request for ${lastRequest.file} (Attempt ${lastRequest.attemptNumber}).`
      );
      ...
      console.log(
        '   -> structural mutation: Tagged file for "Aggressive Decomposition" mode in Executor.'
      );
    } else {
