...
describe('astParser', () => {
  ...
  it('should include location data for imports', async () => {
    ...
    // Specifier location
    const useStateSpec = reactImport.specifiers.find(
      (s) => s.local === 'useState'
    );
  ...
  it('should correctly calculate usage counts (Scope Analysis)', async () => {
    ...
    const useEffectSpec = reactImport.specifiers.find(
      (s) => s.local === 'useEffect'
    );
    expect(useEffectSpec.usageCount).toBe(0);

    // UsedComponent is used in JSX
    const usedCompImport = result.imports.find(
      (i) => i.source === './components/Used'
    );
    ...
    const unusedCompImport = result.imports.find(
      (i) => i.source === './components/Unused'
    );
