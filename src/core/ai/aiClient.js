 ...
 *
 * Note:
 * - Now supports switching between Gemini and Groq via AI_PROVIDER
 */
export async function convertFileWithAI(fileContext, options = {}) {
  ...
  try {
    ...
    if (candidate) {
      try {
        ...
        if (codeMatch && codeMatch[1]) {
          ...
          const depMatch = candidate.match(
            /"dependencies"\s*:\s*\[([\s\S]*?)\]/
          );
          if (depMatch && depMatch[1]) {
            parsed.dependencies = depMatch[1]
              .split(',')
    ...
    if (!parsed.code) {
      ...
      const rawCodeMatch = aiResponse.match(
        /"code"\s*:\s*"([\s\S]*?)"\s*(?:,\s*"dependencies"|\s*})/
      );
      if (rawCodeMatch && rawCodeMatch[1]) {
        parsed.code = rawCodeMatch[1];
        const rawDepMatch = aiResponse.match(
          /"dependencies"\s*:\s*\[([\s\S]*?)\]/
        );
        if (rawDepMatch && rawDepMatch[1]) {
          parsed.dependencies = rawDepMatch[1]
            .split(',')
            .map((d) => d.trim().replace(/^['"]|['"]$/g, ''))
            .filter(Boolean);
        }
  ...
  if (parsed.dependencies && parsed.dependencies.length > 0) {
    console.log(
      `✅ AI Identified Dependencies: ${parsed.dependencies.join(', ')}`
    );
  } else {
